...
def CG(excel='../postflight/20200310_V2.xlsx'):
    ...
    # calculate the ramp mass cg and weight
    rampcg = (payloadmoment + BEM*bemcg + FuelMoment(fuelblock)) /\
                (np.sum(masses) + BEM + fuelblock)
    rampweight = (payloadmoment + BEM*bemcg + FuelMoment(fuelblock))*c.lbstokg*c.g
    cgs = [rampcg]
    ws = [rampweight]
    # calculate the cg for all data points and store them in a list
    [cgs.append((payloadmoment + BEM*bemcg + FuelMoment(fuelblock-fused[i])) /\
                (np.sum(masses) + BEM + fuelblock)) for i in range(len(fused)-1)]
    [ws.append((payloadmoment + BEM*bemcg + FuelMoment(fuelblock-fused[i]))*c.lbstokg*c.g)\
    ...
    cgs.append(shiftcg)
    # return the all the cgs and the shift in cg caused by the passenger movement
    return cgs, (cgs[-2] - cgs[-1])*c.intom, ws

if __name__ == "__main__":
    cgs, cgshift, weights = CG()
    df = pd.DataFrame({"Center of gravity [in]" : cgs, "Weight [N]" : weights})
    df.to_csv("cgs&weights.csv", index=False)