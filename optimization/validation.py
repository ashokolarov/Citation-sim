...
def snakiness():
    ...
    ax1.plot(dat_num[3], dat_num[2], label='Numerical', color=num_color, linewidth=2, linestyle=num_style)
    ax1.plot(fl[5], fl[4], label='Flight data', color=flight_color, linewidth=2, linestyle=flight_style)
    ax1.set_ylabel('Roll rate p[deg/s]', fontsize=18)
    ax1.set_xlabel('Yaw rate r[deg/s]', fontsize=18)
    ax1.set_aspect('equal', 'box')
    ax1.grid(True)

    ax2.plot(dat_num[1], dat_num[0], color=num_color, linewidth=2, linestyle=num_style)
    ax2.plot(fl[3], fl[2], color=flight_color, linewidth=2, linestyle=flight_style)
    ax2.set_ylabel('Roll angle φ[deg', fontsize=18)
    ax2.set_xlabel(r'Sideslip $\beta$[deg]', fontsize=18)
