    ...
    def AsymMode(self, DutchRoll=True, DutchRollYD=True, AperRoll=True, Spiral=True, plotting=False):
        ...
        def MakeArrayAM(t0,te):
            ...
            for i in range(1,len(Yaw)):
                Yaw[i] = quad(interp, dt*(i-1), dt*i)[0]
...
def Trim(data,ts=0,te=999):
    # trimmed = [data[n][(np.where(data[0]<te)) & (np.where(data[0]>ts))] for n in range(len(data))]
    trimmed = [data[n][(data[0] < te) & (data[0] > ts)] for n in range(len(data))]
    return trimmed

def Sample(data,dt=1):
    return [data[n][::dt] for n in range(len(data))]

def EigAper(data, n, ts, te):
    trimmed = Trim(data, ts, te)
    arr = data[n]-data[0][te]
    found = False
    i = -1
    while False:
        i = i+1
...
if __name__ == "__main__":
    ...
    #DR,DRYD,APR,SP
    #VELOCTY,Y,R,YR,RR
    asymdata = dd.AsymMode(0,0,1,1,1)
    # trimmed = Trim(asymdata[0],5,10)
    # print(EigProps(Sample(trimmed,1), 5,'asym'))
